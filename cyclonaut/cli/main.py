...
def _build_cnn_service(model_spec: str, device: str | None) -> Any:
    from cyclonaut.core.models import build_model, get_device
...
def _cmd_cycle_fit(args: argparse.Namespace) -> int:
    from cyclonaut.core.cycle import fit_cyclicity, generate_cycle_plot_data
...
def _cmd_stage(args: argparse.Namespace) -> int:
    if args.engine == "vlm":
        ...
        if args.input.is_dir():
            ...
            from cyclonaut.stages.cnn import BatchClassificationResult
...
def _cmd_gui(args: argparse.Namespace) -> int:
    from cyclonaut.gui.app import main as gui_main
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"cyclonaut: error: {exc}", file=sys.stderr)
        return 1
